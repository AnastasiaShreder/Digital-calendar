    ...
    request.addEventListener('readystatechange', function() {
        if ((request.readyState==4) && (request.status==200)) {
            ...
            for (i=0;i<responce.length;i++){    
                tasks.push({eventName:responce[i].eventName, calendar:responce[i].calendar, color:colors[responce[i].calendar], date:moment(responce[i].date), mark:responce[i].mark, person:responce[i].person, descr:responce[i].descr})
            }
        }
    }) 
